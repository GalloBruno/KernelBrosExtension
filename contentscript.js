...
function kernelbrosEngagementBotActionRunner() {
    ...
    for (var i = 0; i < gblActionsQueue.length; i++) {
        ...
        if (gblActionsQueue[i].enabled == true) {
            ...
            // it's stale, kernelbros wasn't running when the time came
...
var defaultOptions = {
    ...
    dontUnFollowNonKernelBrosEngagementBot: true,
    unFollowFresh: false,
    unFollowIfOld: true,
...
function gbInit() {
    ...
    if (shouldLoadKernelBrosEngagementBotOnThisPage() == false) return false;
    ...
    setInterval(kernelbrosEngagementBotActionRunner, 1000);
...
function outputMessage(txt) {
    ...
    chrome.storage.local.set({
        kernelbrosLog: '' + fakeConsole.textContent + '\n' + txt
    }, function() {});
...
function loadOptions() {
    chrome.storage.local.get("gblOptions", function(data) {
        if (typeof data.gblOptions != 'undefined') {
            ...
            document.getElementById('cbDontUnfollowNonKernelBrosEngagementBot').checked = gblOptions.dontUnFollowNonKernelBrosEngagementBot;
...
function saveOptions() {
    ...
    gblOptions.dontUnFollowNonKernelBrosEngagementBot = document.getElementById('cbDontUnfollowNonKernelBrosEngagementBot').checked;
...
function saveWhiteListToDisk() {
    saveText("kernelbros-whitelist.txt", JSON.stringify(acctsWhiteList));
        ...
        if (this.files && this.files[0]) {
            ...
            reader.addEventListener('load', function(e) {
                ...
                if (fileData.indexOf('relationships_follow_requests_sent') > -1) {
                    ...
                    alert('Loaded partial queue from list of usernames.  Before KernelBrosEngagementBot can use this queue, you will need to process the queue with Get More Data selected.');
                    ...
                    alert("Error: Can't load queue file, are you sure this is a KernelBrosEngagementBot queue?");
                }
...
function handleImagePreload() {
    ...
    images.forEach(image => {
        ...
        $(image).off('click.hideKernelBrosEngagementBotOnOpenPost').on('click.hideKernelBrosEngagementBotOnOpenPost', function() {
                    ...
                    if (data.responseJSON && data.responseJSON.message) {
                        ...
                        if (data.responseJSON.feedback_message) {
                            if (data.responseJSON.feedback_message.indexOf('blocked') > -1) {
                                outputMessage('Message from Instagram (*NOT* KernelBrosEngagementBot): ' + data.responseJSON.feedback_message);
                            }
                        }
    ...
    $.ajax(url)
        .done(function(r) {
            if (r.data.user) {
                ...
                printMessage(' ');
                printMessage(' ');
                outputMessage('Error liking feed - please report to info@kernelbros.com');
                printMessage(' ');
                printMessage(' ');
            }
...
function ajaxUnfollowAcct(acct) {
    ...
    if (gblOptions.dontUnFollowNonKernelBrosEngagementBot === true) {
        var acctFromStorage = alreadyAttempted(acct);
        if (acctFromStorage === false) {
            outputMessage(acct.username + ' was followed outside of KernelBrosEngagementBot, skipping');
            addStamp(acct.id, 'stamp-div-grey', 'non-kernelbros');
            acctsProcessed.push(acct);
            setTimeout(ajaxUnfollowAll, 1);
            return false;
        }
...
function injectIcon() {
    ...
    $('body').prepend('<div id="instabotIcon" title="Hide or Show KernelBrosEngagementBot"></div>');
...
function injectVersionNumber() {
    ...
    document.getElementById('h1KernelBrosEngagementBotHeading').textContent = 'KernelBrosEngagementBot Automator ' + chrome.runtime.getManifest().version + ' for Instagram™';
}
...
function injectControlsDiv() {
    ...
    $.get(chrome.runtime.getURL('kernelbros.html'), function(data) {
        $('body').prepend($.parseHTML(data));
        localizeExtension();

        chrome.storage.local.get("kernelbrosLog", function(data) {
            if (typeof data.kernelbrosLog != 'undefined') {
                var logFromStorage = data.kernelbrosLog;
                var fakeConsole = document.getElementById('txtConsole');
...
async function filterCriteriaMetForUnfollowing(acct) {
    ...
    if (gblOptions.unFollowFresh === false || gblOptions.dontUnFollowNonKernelBrosEngagementBot === true || gblOptions.unFollowIfOld === true) {
        var acctFromStorage = alreadyAttempted(acct);

        if (gblOptions.dontUnFollowNonKernelBrosEngagementBot === true && acctFromStorage === false) {
            outputMessage(acct.username + ' was followed outside of KernelBrosEngagementBot, skipping');
            addStamp(acct.id, 'stamp-div-grey', 'non-kernelbros');
            return false;
        }
    ...
    chrome.runtime.onMessage.addListener(
        function(request, sender, sendResponse) {
            ...
            if (request.igBotUser) {
                ...
                if (window.location.href.indexOf('kernelbros.com.ar') > -1 && window.location.href.indexOf('update_payment_info=true') > -1 && window.location.href.indexOf('guid') == -1) {
            ...
            if (request.openBuyScreen == true && request.igBotUser) {
                ...
                $('#igBotQueueContainer').hide().before('<iframe id="iframePurchase" src="https://www.kernelbros.com.ar/?buyscreen=true&guid=' + guid + '"></iframe>');
                $('.igBotInjectedButton').off('click').addClass('disabled');
                $('li.tab1 label').click();
            }

            if (request.toggleKernelBrosEngagementBot == true) {
                toggleControlsDiv();
            }

            if (request.openKernelBrosEngagementBot == true) {
                openControlsDiv();
            }

            if (request.hideKernelBrosEngagementBot == true) {
...
function getBackgroundInfo() {
    if (user && user.viewer && user.viewer.username != null) {
        ...
        if (extractUserInfoFromScriptTags() === false) {
            //outputMessage('Error loading your account.  You may be temporarily blocked by Instagram, or not loggedin in.  You can try refreshing the page.  If that does not work, please take a break from using KernelBrosEngagementBot for at least 48 hours.');
            return false;
        }
    }
    ...
    $.ajax({
        ...
        .done(function(r) {
            ...
            if (!u.edge_followed_by) {
                outputMessage('Error loading your account.  You may be temporarily blocked by Instagram.  You can try refreshing the page.  If that does not work, please take a break from using KernelBrosEngagementBot for at least 48 hours.');
                return false;
            } else {
...
function shouldLoadKernelBrosEngagementBotOnThisPage() {
    ...
    if (shouldLoad === false) {
        console.log('Cannot load KernelBrosEngagementBot on this page');
